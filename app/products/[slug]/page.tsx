...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="inner-page product-page">
      <SiteHeader />
      <section className="inner-hero product-hero"><div><nav className="breadcrumbs dark" aria-label="面包屑"><a href="/">首页</a><span>/</span><a href="/products">产品中心</a><span>/</span><b>{product.title}</b></nav><p>PRODUCTS · {product.n}</p><h1>{product.title}</h1><span>{product.en}</span></div></section>
      <section className="product-overview">
        ...
        <div className="product-copy"><p className="kicker">PRODUCT OVERVIEW</p><h2>{product.title}</h2><p>{product.desc}</p><div className="feature-row">{product.features.map((feature, i) => <span key={feature}><b>0{i + 1}</b>{feature}</span>)}</div><a href="/#contact">联系优能创 <b>↗</b></a></div>
      </section>
